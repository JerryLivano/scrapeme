        ...
        switch (toastType) {
            ...
            case "error-email":
                toastProperties = {
                    ...
                    title: (
                        <div style={{ color: "red" }}>
                            Email/Password not registered
                        </div>
                    ),
                    description:
                        "Input the account registered by the administrator",
                    textColor: "text-red-500",
                    borderColor: "none",
                    icon: XCircleIcon,
                };
            ...
            case "error":
                toastProperties = {
                    ...
                    description:
                        "Input email (@mii.co.id) and password correctly",
                    textColor: "text-red-500",
                    borderColor: "none",
                    icon: XCircleIcon,
                };
                ...
                <div className='flex'>
                    <div className='absolute top-0 right-0 mr-4 mt-4 w-[370px]'>
                        {listToast.map((toast, i) => (
                            <div
                                key={i}
                                className={`border ${toast.borderColor} bg-white p-4 rounded-lg shadow`}
                            >
                                <div className='flex'>
                                    <toast.icon
                                        className={`${toast.textColor} w-6 h-5 mt-1`}
                                    />
                                    <h4 className='ml-2 font-semibold'>
                                        {toast.title}
                                    </h4>
                                    <button
                                        className='flex ml-auto text-slate-400'
                                        onClick={() => handleExit()}
                                    >
